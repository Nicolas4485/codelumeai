...
/**
 * Generate a CLAUDE.md file at the workspace root so Claude Code has rich
 * codebase context on every session — foundations, entry points, top symbols,
 * key cross-file dependencies, and the cached AI briefing if available.
 */
async function generateClaudeMd(state: ExtensionState): Promise<void> {
  ...
  if (!state.graphStore) {
    void vscode.window.showWarningMessage(
      "CodeLumeAI: Index the workspace first (CodeLumeAI: Index Workspace for Connections) before generating CLAUDE.md.",
    );
    return;
  }
  ...
  if (s.files === 0) {
    void vscode.window.showWarningMessage(
      "CodeLumeAI: Workspace has not been indexed yet. Run CodeLumeAI: Index Workspace for Connections first.",
    );
    return;
  }

  // ── Gather data from the graph store ────────────────────────────────────
  const fileStats = store.getFileStats().filter((f) => f.symbolCount > 0);
  const maxIn = Math.max(...fileStats.map((f) => f.incomingRefs), 1);
  const foundations = fileStats
    .filter((f) => f.incomingRefs >= Math.max(2, Math.ceil(maxIn * 0.4)))
    .slice(0, 15);
  const entryPoints = fileStats
    .filter((f) => f.incomingRefs === 0 && f.outgoingDeps > 0)
    .slice(0, 10);
  const topSymbols = store.getTopSymbols(20);
  const topEdges = store.getGraphEdges(30);

  // ── Pull cached AI briefing if available ────────────────────────────────
  const briefingCacheKey = `codelumeai.briefing.${GraphStore.workspaceHash(
    (state.workspaceFolderUri as vscode.Uri).fsPath,
  )}`;
  const briefing = state.context.globalState.get<{
    headline: string;
    overview: string;
    architectureNotes: string;
    keyConcepts: Array<{ term: string; definition: string; file?: string }>;
  ...
  lines.push(`# CLAUDE.md — ${workspaceName} codebase map`);
  lines.push(`_Auto-generated by [CodeLumeAI](https://marketplace.visualstudio.com/items?itemName=nic-ai.codelumeai) on ${now}._`);
  lines.push(`_Re-run \`CodeLumeAI: Generate CLAUDE.md\` to refresh after indexing or adding files._`);
  lines.push("");
  lines.push(`**Index:** ${s.files} files · ${s.symbols} symbols · ${s.refs} cross-file references`);
  lines.push("");
  lines.push("---");
  lines.push("");

  // Project overview from briefing
  if (briefing) {
    lines.push("## What this project does");
    lines.push("");
    lines.push(briefing.headline);
    lines.push("");
    lines.push(briefing.overview);
    lines.push("");
    if (briefing.architectureNotes) {
      lines.push("## Architecture");
      lines.push("");
      lines.push(briefing.architectureNotes);
      lines.push("");
    }
  }
  ...
  if (foundations.length > 0) {
    lines.push("## Foundation files — read and understand these first");
    lines.push("");
    lines.push(
      "These files are depended on by the most other files. " +
      "Changing them has the widest blast radius. Read them before touching anything else.",
    );
    lines.push("");
    for (const f of foundations) {
      lines.push(`- \`${f.path}\` — ${f.incomingRefs} file(s) depend on this · ${f.symbolCount} symbols · ${f.outgoingDeps} outgoing deps`);
    }
    lines.push("");
  }
  ...
  if (entryPoints.length > 0) {
    lines.push("## Entry points — where execution starts");
    lines.push("");
    for (const f of entryPoints) {
      lines.push(`- \`${f.path}\` — ${f.outgoingDeps} outgoing deps · ${f.symbolCount} symbols`);
    }
    lines.push("");
  }
  ...
  if (topSymbols.length > 0) {
    ...
    lines.push("");
    lines.push(
      "These functions, classes, and types are used across the most files. " +
      "Understand these before making broad changes.",
    );
    lines.push("");
    for (const sym of topSymbols) {
      lines.push(`- \`${sym.name}\` (${sym.kind}) in \`${sym.file}\` — referenced by ${sym.totalRefs} file(s)`);
    }
    lines.push("");
  }
  ...
  if (topEdges.length > 0) {
    lines.push("## Key cross-file dependencies");
    lines.push("");
    lines.push("Strongest import relationships in the codebase (weight = number of symbol references):");
    lines.push("");
  ...
  // Key concepts from briefing
  if (briefing?.keyConcepts && briefing.keyConcepts.length > 0) {
    lines.push("## Key concepts and domain terms");
    lines.push("");
    for (const concept of briefing.keyConcepts) {
      const fileNote = concept.file ? ` (see \`${concept.file}\`)` : "";
      lines.push(`- **${concept.term}**${fileNote} — ${concept.definition}`);
    }
    lines.push("");
  }

  // Start-here hint
  if (briefing?.startHereFile) {
    lines.push("## Start here");
    lines.push("");
    lines.push(`Open \`${briefing.startHereFile}\` first — it is the best entry point for understanding the codebase as a whole.`);
    lines.push("");
  }
  ...
  lines.push("---");
  lines.push("");
  lines.push("## Tips for Claude Code");
  lines.push("");
  lines.push("- Before editing a foundation file, check what depends on it — changes ripple widely.");
  lines.push("- The entry points listed above are the best place to trace execution flow.");
  lines.push("- Use `CodeLumeAI: Show Dependency Graph` in VS Code to visually explore connections.");
  lines.push("- Use `CodeLumeAI: Open Side Panel` to read any file in plain English.");
  lines.push("");

  // ── Write to disk ────────────────────────────────────────────────────────
  const claudeMdPath = vscode.Uri.joinPath(
    state.workspaceFolderUri as vscode.Uri,
    "CLAUDE.md",
  );
  const content = lines.join("\n");
  await vscode.workspace.fs.writeFile(claudeMdPath, Buffer.from(content, "utf8"));

  log(state, "info", `CLAUDE.md written to ${claudeMdPath.fsPath} (${content.length} chars)`);

  const open = await vscode.window.showInformationMessage(
    `CodeLumeAI: CLAUDE.md generated (${s.files} files, ${s.symbols} symbols). Claude Code will now use this as project context automatically.`,
    "Open CLAUDE.md",
  );
  if (open) {
    await vscode.window.showTextDocument(claudeMdPath);
  }
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("codelumeai.generateClaudeMd", async () => {
